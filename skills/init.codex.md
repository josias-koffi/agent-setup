...
```bash
test -x "${CODEX_HOME:-$HOME/.codex}/agent-setup/bin/render-templates.sh"   || { echo "Framework not installed. Run: bash bootstrap.sh (from the agent-setup repo)" >&2; exit 2; }
```
