...
3. **No destructive changes to existing code** — you may add files under `.claude/`, `.project/`, `agents/`, `workflows/`, `skills/`, `sprints/`, `spec/`. You may NEVER modify source code or config outside those directories unless the user explicitly asks.
...
```bash
test -x "$HOME/.claude/agent-setup/bin/render-templates.sh" \
...
- Admin dashboard (src/features/admin/)" \
  ...
  "$HOME/.claude/agent-setup/bin/render-templates.sh" "$(pwd)"
```
...
if [ -f README.md ]; then
    if ! grep -q '<!-- generated-by: /init -->' README.md; then
        ...
        sed "s/{{PROJECT_NAME}}/$PROJECT_NAME/g" \
            "$HOME/.claude/agent-setup/templates/readme-append.md.tpl" \
            >> README.md
    fi
else
